            ...
            <DeleteConfirmDialog
                ...
                description={
                    detail.session.continuationSessionIds.length > 1
                        ? 'Permanently delete this Kiro conversation from disk. This removes the parent session, every recognized continuation segment, and matching execution files.'
                        : 'Permanently delete this Kiro session segment from disk. This removes its session JSON file and matching execution files.'
                }
                errorMessage={
...
export const Route = createFileRoute('/kiro-sessions/$sessionId')({
    ...
    loader: ({ context, params }) => {
        return context.queryClient.ensureQueryData(kiroSessionDetailQueryOptions(params.sessionId));
    },
    pendingComponent: () => (
        <LoadingPanel
