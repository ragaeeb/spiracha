...
test('should import a chat, hydrate its transcript, and survive a direct reload', async ({ page }) => {
    await page.goto('/web');
    await page.getByLabel('Import web chat JSON files').setInputFiles(
        jsonFile('browser-chat.json', chat('e2e-single', 'E2E single import')),
    );
    await expect(page).toHaveURL(/\/web-chats\//u);
    await expect(page.getByRole('heading', { name: 'E2E single import', exact: true })).toBeVisible();
