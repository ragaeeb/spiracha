...
type CommandCodeWorkspacesTableProps = {
    workspaces: CommandCodeWorkspaceGroup[];
};

const columnHelper = createDataTableColumnHelper<CommandCodeWorkspaceGroup>();
...
export function CommandCodeWorkspacesTable({ workspaces }: CommandCodeWorkspacesTableProps) {
    return (
        <DataTable
            columns={columns}
            data={workspaces}
            emptyMessage="No Command Code workspaces match the current search."
        />
    );
}
