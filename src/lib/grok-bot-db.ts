...
// The app owns its synchronized persistence; delete the bot/group at the gateway.
export const deleteGrokBotConversation = async (persistenceDir: string, conversationId: string) => {
    const deleted = await deleteGrokBotAgent(persistenceDir, conversationId);
    return { deletedFiles: [], deletedIds: deleted ? [conversationId] : [] };
};
