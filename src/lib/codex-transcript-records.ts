...
export const createEmptySessionMeta = (): SessionMetaExtended => {
    return {
...
export const parseCodexTranscriptRecords = (
...
export const createTranscriptState = (options: ParseCodexTranscriptOptions): ParseCodexTranscriptState => ({
    assistantModel: null,
    eventFilter: options.eventFilter ?? (() => true),
    events: [],
    includeRaw: options.includeRaw ?? true,
    maxEvents: options.maxEvents ?? Number.POSITIVE_INFINITY,
    maxTurnContexts: options.maxTurnContexts ?? Number.POSITIVE_INFINITY,
    modelNames: [],
    sequence: 0,
    shouldStop: false,
    tailEventCursor: 0,
    tailEventLimit: options.tailEventLimit ?? Number.POSITIVE_INFINITY,
    tailEventsWrapped: false,
    turnContexts: [],
});

export const consumeTranscriptRecord = (
    parsed: Record<string, JsonValue>,
    state: ParseCodexTranscriptState,
...
export const finalizeTranscript = (
    ...
    if (state.tailEventsWrapped && state.events.length > 0) {
        state.events.splice(
            0,
            state.events.length,
            ...state.events.slice(state.tailEventCursor),
            ...state.events.slice(0, state.tailEventCursor),
        );
    }
    ...
    stats.modelNames = state.modelNames;
